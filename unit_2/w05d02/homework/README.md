...
- <details><summary>Expected File Structure</summary>
  ...
  ![file structure image](https://i.imgur.com/ssyJHrk.png)
  
  **IMPORTANT! Ignore public folder - we did not cover it today (sorry I didn't have time to update the image!) <br>
  ALSO Your 'database' was in a models folder in today's lesson. Since we only have one file, it is ok to not make the models folder and just match this file structure for today. It is also ok to make the folder - go with what makes sense for you
  **
