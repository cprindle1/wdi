//Thanks to Dylan Williams for this great solution! 
...
$(function(){
  ...
  $h1 = $('<h1>');
  $h1.text('Hogwarts');
  // h1 isn't visible because it hasn't yet been appended
  $('body').append($h1);

  // YEAR 2
  $('<h2>').text('Dylan Williams').appendTo($('body'));
  $('<h3>').text('Ravenclaw').appendTo($('body'));
  $('<h4>').text('Corvus').addClass('crow').appendTo($('body')); // font-family: fantasy doesn't work?
  $('<h4>').text('Holly Wand with Unicorn Hair Core').appendTo($('body'));

  // YEAR 3
  var $ul = $('<ul>').attr('storage','trunk');
